...
const allowPaid = option("allow-paid-provider", "") === providerName;
...
const provider = createCompatibleProvider({ provider: providerName, model, apiKey: process.env[key], timeoutMs: Number(option("timeout-ms", "120000")), maxTokens: Number(option("max-tokens", "5000")), allowPaid });
try {
  ...
  console.log(JSON.stringify({ run_id: result.run_id, status: result.status, packets: result.packets.map(({ packet, storage_status, material_status }) => ({ packet_id: packet.packet_id, packet_path: packets.packetPath(packet.packet_id), source_label: packet.subject.source_label, material_status, storage_status, page_count: packet.pages.length })), budget: result.budget, state_root: stateRoot, ledger_path: path.join(stateRoot, "scout-ledger.sqlite"), manifest_path: manifestPath, report: reportPath ? path.resolve(reportPath) : null }, null, 2));
